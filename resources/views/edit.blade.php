...
<body>
    <div class="container w-50">
        <form class="form-group" action="{{ route('users.update', $model->id) }}" method="post">
            @csrf
            @method("PUT")
            <div class="mt-3">
                ...
                <select class="form-select" name="phongban_id">
                    @foreach ($departments as $department)
                        <option value="{{ $department->id }}"
                            {{ $department->id == $model->phongban_id ? 'selected' : '' }}>
            ...
            <div class="mt-3">
                ...
                <input class="form-control" type="number" name="age" value="{{ $model->tuoi }}">
            </div>
